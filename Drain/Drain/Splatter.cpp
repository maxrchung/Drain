...
int Splatter::make() {
    ...
    for (int i = 1; i < numDrops; i++) {
        ...
        case 3:
			...
			// 0.70f because the center circle starts from 0.8 size then goes to 1.0
            auto dist = (expRandomPoint + sizeFactor / 2.0f * 0.70f) * SPRITE_SIZE;   // dist away from centerPoint the satellite will be drawn
...
void Splatter::make(const Time& startTime,
	...
	Splatter(startTime, endTime, size, 400, 3, spawnTime, bubble).make();
}
...
std::vector<SpriteCollection> Splatter::renderSecondAllPop(std::vector<Bubble*>& bubbles) {
	...
	for (const auto bubble : bubbles) {
		make(splatterTime, splatterTime, 0, bubble);
	}
	return splatters;
}